...
linear_dep_R_2 = [2 * math.sqrt(2) * math.pi, math.sqrt(2), math.pi]
...
indep_vec_1 = Vec({0, 1, 2, 3},{0: one, 1: 0, 2: 0, 3: one})
indep_vec_2 = Vec({0, 1, 2, 3},{0: 0, 1: one, 2: 0, 3: 0})
indep_vec_3 = Vec({0, 1, 2, 3},{0: 0, 1: 0, 2: one, 3: 0})
indep_vec_4 = Vec({0, 1, 2, 3},{0: 0, 1: 0, 2: 0, 3: one})
...
def is_superfluous(L, i):
    ...
    elem = L2[i]
    L2.pop(i)
    soln = vec2rep(L2, elem)
    # check if solution is an actual solution?
    residual = elem - coldict2mat(L2) * soln
    return residual * residual < 10**-14
