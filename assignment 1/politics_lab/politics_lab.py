...
def create_voting_dict():
    ...
    for vote_entry in voting_data:
        ...
        for one in ones_list:
            new_ones_list.append(int(one))

        dict[v[0]] = new_ones_list
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    if sen_a in voting_dict.keys() and sen_b in voting_dict.keys():
        list_a = voting_dict[sen_a]
        list_b = voting_dict[sen_b]

        for i in range(len(list_a)):
            dotSum += list_a[i] * list_b[i]
...
def least_similar(sen, voting_dict):
    ...
    for key, val in voting_dict.items():
        if key != sen:
            ...
            if least_sim > cmp:
                least_sim = cmp
                least_sim_senator = key
    return least_sim_senator
...
def find_average_record(sen_set, voting_dict):
    ...
    return [ a / len(sen_set)for a in avg_record]
